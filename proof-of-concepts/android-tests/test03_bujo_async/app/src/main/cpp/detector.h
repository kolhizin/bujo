...
namespace bujo
{
	namespace detector
	{
		...
		public:
			...
			void loadImage(const xt::xtensor<float, 2>& src, float scale = 1.0f);
			float detectAngle(float max_text_angle = 1.57f);
			void alignImages();
			void filterImages(const FilteringOptions& options = FilteringOptions());
